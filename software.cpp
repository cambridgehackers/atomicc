...
int generateSoftware(std::list<ModuleIR *> &irSeq, const char *exename, std::string outName)
{
    ...
    if (softwareNameList.size() > 0) {
        ...
        IR->fields.push_back(FieldElement{localName, -1, dutType, false, false, false, false});
        if (hasPrintf) {
#if 0
...
printf("[%s:%d] HASHSHSHSHSPRINTF %p\n", __FUNCTION__, __LINE__, muxDef);
            if (muxDef)
                ...
                muxDef->interfaces.push_back(FieldElement{"in", -1, pipeName, false, false, false, false});
                muxDef->interfaces.push_back(FieldElement{"forward", -1, pipeName, false, false, false, false});
                muxDef->interfaces.push_back(FieldElement{"out", -1, pipeName, true, false, false, false});
...
dumpModule("MUX", muxDef);
...
#endif
            IR->fields.push_back(FieldElement{muxName, -1, muxTypeName, false, false, false, false});
        }
        ...
        for (auto item: softwareNameList) {
            ...
            ifcIR->interfaces.push_back(FieldElement{"method", -1, userTypeName, !outcall, false, false, false});
            ifcIR->interfaces.push_back(FieldElement{"pipe", -1, pipeName, outcall, false, false, false});
            IR->fields.push_back(FieldElement{fieldName, -1, ifcIR->name, false, false, false, false});
            IR->interfaces.push_back(FieldElement{userInterface, -1, pipeName, outcall, false, false, false});
