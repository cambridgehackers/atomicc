...
In summary, AtomicC is a way of constructing highly parallel hardware artifacts
without being burdened with the mechanics of handling concurrent consistency.
All parallel operations are checked for safety by the compiler and errors
robustly flagged.
...
is nothing the compiler can do to help.  With that in mind, the automation
of the tedious checking for concurrent execution conflicts allows
the user to focus solely on the algorithm to be implemented.
