...
is enabled.
To preserve the standard interpretation of C++ assigment,
can use the following mental model of operations:
\begin{enumerate}
...
\end{codeblock}
\end{enumerate}
This processing style is described in more detail as \textbf{modification in-private}
in Prinz\cite[Sec.~3.2]{Prinz03operationalsemantics}.
...
In summary, although AtomicC is a way of constructing highly parallel hardware artifacts,
the user model is quite simple:
\begin{itemize}
\item Methods/rules are written as though they were serially executed,
...
\item Consistent concurrent execution of transactions is guaranteed by the
compiler framework,
\item Consistent concurrent implementation of the code for methods/rules
is guaranteed by the compiler's
translation of the SSA source text into combinatoric (only) logic.
\end{itemize}
...
is nothing the compiler can do to help.  With that in mind,
no being burdened with the mechanics of handling concurrent consistency,
the user can efficiently focus solely on the algorithm development itself.
